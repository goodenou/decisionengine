...
class Worker:
    ...
    def __init__(self, conf_dict, base_class):
        ...
        delogger.debug(f"Creating worker: module={self.module} name={self.name} parameters={conf_dict['parameters']} schedule={self.schedule}")
...
class TaskManager:
    ...
    def set_loglevel_value(self, log_level):
        ...
        with self.loglevel.get_lock():
            # Convert from string to int using technique
